...
# def alpine_dante():
#     subprocess.run(['/usr/bin/alpine-dante', 'tendermint'])

# def sock_d():
#     subprocess.run(['/usr/local/bin/dumb-init', '/usr/local/bin/sockd'])

if __name__ == '__main__':
    print('')
    account_name = raw_input('Please enter account name: ')
    account_password = raw_input('Please enter account password: ')
    ton = raw_input('Do you want to share your bandwidth for Telegram Proxies? [Y/n] ')
    print('')
    ...
    if ton == 'y' or ton == 'Y':
        subprocess.Popen(['nohup', '/usr/bin/alpine-dante', 'tendermint', '>>', '/dev/null', '&'])
